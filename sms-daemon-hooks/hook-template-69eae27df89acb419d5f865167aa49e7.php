 ...
 *    ["text"]=>string(4) "KEMBALI"
 *    ["keyword"]=>string(4) "KEMBALI"
 *    ["status"]=>NULL
 *    ["params"]=>array(1) 
 *    {
 *      [0]=>string(4) "KEMBALI"
 ...
 * 
 */
$my_kembali_kategori = 'Inkubator bayi';
$my_kembali_keyword = 'KEMBALI';
$my_kembali_description = 'SMS Pengembalian Inkubator';
$my_kembali_sms_format = 'KEMBALI*KODEPINJAM*CM_PANJANGBAYI*CM_BERATBAYI*<SEHAT/SAKIT>';
$my_kembali_sms_sample = 'KEMBALI*323431-353131-35*30*3.60*SEHAT';
 ...
 */
function my_hook_kembali_function($keyword, $params)
{
    ...
    else
    {
        if (count($params['params'])!=$valid_param_count){
            ...
            sms_send($params['sender'], '2/2. Format SMS: '.$my_kembali_sms_format, $nama_modem);
            sms_send($params['sender'], '3/2. Contoh SMS: '.$my_kembali_sms_sample, $nama_modem);
        }
        else
        {
            ...
            {         
                ...
                else
                {
                    ...
                    $save_sql = "insert into inkubator_kembali (
                        id, kode_pinjam, id_inkubator, berat_kembali, panjang_kembali, kondisi_kembali     
                    ) values (
                        $id_pinjam, '$kode_pinjam', (select p.id_inkubator from inkubator_pinjam p where p.kode_pinjam = '$kode_pinjam'), $p_berat, $p_pjg, '$p_kondisi'
                    )";
                    $sub_mon_sql = "insert into inkubator_monitoring 
                	   ...
                	   values
                		...
                		concat('Status akhir ', (select p.nama_bayi from inkubator_pinjam p where p.kode_pinjam = '$kode_pinjam'))
                	   );";
                    /*
                    ...
                    */                    
                    if (exec_query($save_sql))
                    {
                        ...
                        {
                            sms_send($params['sender'], 'Pengembalian sedang diproses. Silahkan antar inkubator dan perlihat Kode Pinjam: '.$kode_pinjam, $nama_modem);
                        }
                        else
                        {
                            exec_query("delete from inkubator_kembali where id = $id_pinjam");
 ...
 */
function my_hook_kembali_register_callback_function($keyword)
{
 ...
 *  - Keyword: your keyword.
 */
function my_hook_kembali_unregister_callback_function($keyword)
{
 ...
 *  - Keyword: your keyword.
 */
function my_hook_kembali_activation_callback_function($keyword)
{
    // create your table here, etc., and...
 ...
 *  - Keyword: your keyword.
 */
function my_hook_kembali_deactivation_callback_function($keyword)
{
    // drop your table here, etc., and...
...
/**
 * Register your keyword kembali and its hook function to database. 
...
keyword_hook_register(
    $my_kembali_keyword, 
    'my_hook_kembali_function', // hook function name.
    __FILE__, // current file.
    $my_kembali_description, 
    $my_kembali_sms_format, 
    $my_kembali_sms_sample,
    $my_kembali_kategori
);
*/
/**
